...
| Reproducibility | Fixed seed across `random`, `numpy`, `torch` | Same split and results on every run |
...
| Specificity (normal recall) | 0.581 |
| Accuracy | 0.840 |
| F1 (Pneumonia) | 0.886 |
| F1 (Normal) | 0.731 |

Confusion matrix (test, n = 624): of 390 true pneumonia cases the model missed only **2**; of 234 true normals it correctly cleared 136.

![Evaluation](evaluation.png)

The strong AUROC (0.956) shows the model genuinely separates the two classes. At the default threshold it behaves as a **high-sensitivity screener**: it rarely misses pneumonia, at the cost of over-calling it on some normal X-rays.
...
A natural next step is to tune the decision threshold to rebalance sensitivity and specificity. Selecting the threshold on the validation set (Youden's J) and applying it to the test set **did not improve** test specificity.

![Operating points](threshold_tuning.png)

The reason is instructive: the validation split is drawn from the (easier) training distribution, where normal X-rays score very low, whereas the test set's normals score higher. An operating point calibrated on one distribution does not transfer to a shifted one. This is a deliberate finding rather than a fixable bug — correcting it would require a validation set drawn from the *deployment* distribution, and tuning the threshold on the test set itself would be a methodological error. The default high-sensitivity operating point is therefore reported as-is.
